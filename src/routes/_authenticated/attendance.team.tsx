...
export const Route = createFileRoute("/_authenticated/attendance/team")({
  ...
  validateSearch: (s: Record<string, unknown>): AttSearch => ({
    ...
    view: ["all", "logged_in", "present", "absent"].includes(s.view as string) ? (s.view as AttSearch["view"]) : undefined,
  }),
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">{isAdmin ? "All employees" : "Your team"} — review attendance, corrections and leaves.</p>
      </div>
      ...
      <Tabs defaultValue="attendance">
                ...
                <TableRow key={r.id}>
                  ...
                  <TableCell>
                    {canRequestCorrection && (
                      <CorrectionDialog record={r} onChange={() => qc.invalidateQueries({ queryKey: ["corrections"] })} />
                    )}
                  </TableCell>
                </TableRow>
              ))}
              {rows && rows.length === 0 && (
                <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground py-8">No records in range</TableCell></TableRow>
              )}
            </TableBody>
        ...
        <TabsContent value="corrections">
          <Card><CardContent className="p-0"><Table>
            <TableHeader><TableRow>
              <TableHead>Date</TableHead><TableHead>Employee</TableHead>
              <TableHead>Requested Login</TableHead><TableHead>Requested Logout</TableHead>
                ...
                <TableRow key={c.id}>
                  ...
                  <TableCell>
                    {canReview && c.status === "pending" && (
                      <ReviewButtons kind="correction" id={c.id} extra={{ correction: c }} onDone={() => { qc.invalidateQueries({ queryKey: ["corrections"] }); qc.invalidateQueries({ queryKey: ["team-att"] }); }} />
                    )}
                  </TableCell>
                </TableRow>
              ))}
              {corrections && corrections.length === 0 && (
                <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground py-8">No correction requests</TableCell></TableRow>
              )}
            </TableBody>
          </Table></CardContent></Card>
        </TabsContent>
                ...
                <TableRow key={l.id}>
                  ...
                  <TableCell>
                    {canReview && l.status === "pending" && (
                      <ReviewButtons kind="leave" id={l.id} onDone={() => qc.invalidateQueries({ queryKey: ["team-leaves"] })} />
                    )}
                  </TableCell>
                </TableRow>
              ))}
  ...
  const submit = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("attendance_corrections").insert({
        ...
        requested_status: (form.requested_status || null) as "present" | "half_day" | "absent" | "leave" | "weekly_off" | "holiday" | null,
        reason: form.reason.trim(),
        requested_by: employee.id,
      });
      if (error) throw error;
    },
    onSuccess: () => { toast.success("Correction requested"); setOpen(false); onChange(); },
      ...
      <DialogContent>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><Label>Logout</Label><Input type="datetime-local" value={form.requested_logout_at} onChange={(e) => setForm({ ...form, requested_logout_at: e.target.value })} /></div>
          </div>
          <div>
            <Label>Override status (optional)</Label>
            <Select value={form.requested_status} onValueChange={(v) => setForm({ ...form, requested_status: v as typeof form.requested_status })}>
              <SelectTrigger><SelectValue placeholder="(auto from hours)" /></SelectTrigger>
              <SelectContent>
        ...
        <DialogFooter><Button onClick={() => submit.mutate()} disabled={submit.isPending}>Submit</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  );
}

function ReviewButtons({ kind, id, extra, onDone }: { kind: "leave" | "correction"; id: string; extra?: { correction?: CorrRow }; onDone: () => void }) {
  const { employee } = useAuth();
  ...
  const submit = useMutation({
    mutationFn: async () => {
      ...
      if (kind === "leave") {
        const { error } = await supabase.from("leave_requests").update({
          status: action, reviewed_by: reviewer, reviewed_at: now, review_notes: notes || null,
        }).eq("id", id);
        if (error) throw error;
      } else {
    ...
    onSuccess: () => { toast.success("Submitted"); setOpen(false); setNotes(""); onDone(); },
    onError: (e: Error) => toast.error(e.message),
  });
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <div className="flex gap-1">
        <Button size="sm" onClick={() => { setAction("approved"); setOpen(true); }}>Approve</Button>
        <Button size="sm" variant="destructive" onClick={() => { setAction("rejected"); setOpen(true); }}>Reject</Button>
      </div>
      <DialogContent>
        <DialogHeader><DialogTitle>{action === "approved" ? "Approve" : "Reject"} request</DialogTitle></DialogHeader>
        <Textarea placeholder="Review notes (optional)" rows={3} maxLength={500} value={notes} onChange={(e) => setNotes(e.target.value)} />
        <DialogFooter><Button onClick={() => submit.mutate()} disabled={submit.isPending}>Confirm</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  );
}