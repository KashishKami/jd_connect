...
CREATE OR REPLACE FUNCTION public.agent_rankings(
  _from date,
  _to date,
...
AS $$
  WITH s AS (
    ...
    WHERE sale_date BETWEEN _from AND _to
    GROUP BY employee_id
  ),
  r AS (
    SELECT employee_id, SUM(amount_usd) amt
  ...
  top_rows AS (
    ...
    FROM base b
    ORDER BY b.net_revenue DESC NULLS LAST
    LIMIT GREATEST(1, LEAST(_limit, 50))
  ),
  bottom_rows AS (
    SELECT b.*, 'bottom'::text AS rank_position
    FROM base b
    WHERE b.employee_id NOT IN (SELECT employee_id FROM top_rows)
    ORDER BY b.net_revenue ASC NULLS LAST
    LIMIT GREATEST(1, LEAST(_limit, 50))
  )
...
GRANT EXECUTE ON FUNCTION public.agent_rankings(date, date, uuid, int) TO authenticated;
GRANT EXECUTE ON FUNCTION public.agent_rankings(date, date, uuid, int) TO service_role;
